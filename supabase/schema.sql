...
create policy "Faculty can read own profile" on public.faculties
for select using (lower(email) = public.current_user_email() and is_active = true);

create policy "Admins manage subjects" on public.subjects
for all using (public.is_admin()) with check (public.is_admin());

create policy "Faculty can read assigned subjects" on public.subjects
for select using (faculty_id = public.current_faculty_id());

create policy "Admins manage students" on public.students
for all using (public.is_admin()) with check (public.is_admin());

create policy "Faculty can read students" on public.students
for select using (public.current_faculty_id() is not null);

create policy "Admins manage attendance" on public.attendance_records
for all using (public.is_admin()) with check (public.is_admin());

create policy "Faculty read assigned attendance" on public.attendance_records
for select using (public.is_faculty_for_subject(subject_id));

create policy "Faculty create assigned attendance" on public.attendance_records
for insert with check (public.is_faculty_for_subject(subject_id));

create policy "Faculty update assigned attendance" on public.attendance_records
for update using (public.is_faculty_for_subject(subject_id)) with check (public.is_faculty_for_subject(subject_id));

create policy "Faculty delete assigned attendance" on public.attendance_records
for delete using (public.is_faculty_for_subject(subject_id));
...
-- After creating the first Supabase Auth admin user, register the email here:
